...
class AIService:
    ...
    def generate_response(self, message: str, conversation_history: list = None, user=None) -> Dict[str, Any]:
...
Try rephrasing your question with specific business context or terminology from our playbooks — I'm here to deliver executive-level insights!"""
    ...
    def _get_user_info(self, user) -> dict:
        """Get user information from ClientInfo model.
        
        Args:
            user: User instance
            
        Returns:
            Dictionary containing user business information
        """
        if not user:
            return {}
        
        try:
            # Get ClientInfo for the user
            client_info = getattr(user, 'client_info', None)
            ...
            return {
                "id": client_info.id,
                "company_name": client_info.company_name or "",
                "owner_name": client_info.owner_name or "",
                "state": client_info.state or "",
                "city": client_info.city or "",
                "year_started": client_info.year_started or 2000,
                "trucks_count": client_info.trucks_count or 5,
                "monthly_revenue": client_info.monthly_revenue or "0-250k",
                "gross_profit_margin": str(client_info.gross_profit_margin or "50.00"),
                "main_services": client_info.main_services or [
                ...
                "pricing_model": client_info.pricing_model or "by_weight",
                "software_tools": client_info.software_tools or [
                    "CRM Software",
                    "GPS Tracking",
                    "Route Optimization",
                ...
                "current_challenges": client_info.current_challenges or "How to retain employees?"
            }
